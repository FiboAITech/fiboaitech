...
from fiboaitech.nodes.managers import NodeManager
from fiboaitech.nodes.node import ConnectionNode, NodeDependency
from fiboaitech.prompts import Prompt
from fiboaitech.serializers.types import WorkflowYamlData
...
class WorkflowYAMLLoader:
    ...
    def get_workflows(cls, data: dict, flows: dict[str, Flow]) -> dict[str, Workflow]:
        ...
        for wf_id, wf_data in data.get("workflows", {}).items():
            if not (flow_id := wf_data.get("flow")):
                raise WorkflowYAMLLoaderException(f"Value 'flow' for fiboaitech '{wf_id}' not found ")
            if not (flow := flows.get(flow_id)):
                raise WorkflowYAMLLoaderException(f"Flow '{flow_id}' for fiboaitech '{wf_id}' not found")
    ...
    def parse(
        ...
        """
        Parse fiboaitech workflow data.
