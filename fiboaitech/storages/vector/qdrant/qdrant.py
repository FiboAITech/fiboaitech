...
from fiboaitech.storages.vector.exceptions import VectorStoreDuplicateDocumentException as DuplicateDocumentError
from fiboaitech.storages.vector.exceptions import VectorStoreException as DocumentStoreError
from fiboaitech.storages.vector.policies import DuplicatePolicy
from fiboaitech.storages.vector.qdrant.converters import (
    DENSE_VECTORS_NAME,
    SPARSE_VECTORS_NAME,
    convert_fiboaitech_documents_to_qdrant_points,
    convert_id,
    convert_qdrant_point_to_fiboaitech_document,
)
from fiboaitech.storages.vector.qdrant.filters import convert_filters_to_qdrant
from fiboaitech.storages.vector.utils import create_file_id_filter
...
class QdrantVectorStore:
    ...
    def filter_documents(
        ...
        [documentation](https://docs.fiboaitech.deepset.ai/docs/metadata-filtering)
    ...
    def write_documents(
        ...
        for document_batch in batched_documents:
            batch = convert_fiboaitech_documents_to_qdrant_points(
                document_batch,
                use_sparse_embeddings=self.use_sparse_embeddings,
            )
    ...
    def get_documents_generator(
        ...
        while not stop_scrolling:
            ...
            for record in records:
                yield convert_qdrant_point_to_fiboaitech_document(record, use_sparse_embeddings=self.use_sparse_embeddings)
    ...
    def get_documents_by_id(
        ...
        for record in records:
            documents.append(
                convert_qdrant_point_to_fiboaitech_document(record, use_sparse_embeddings=self.use_sparse_embeddings)
            )
        return documents
    ...
    def _query_by_sparse(
        ...
        results = [
            convert_qdrant_point_to_fiboaitech_document(point, use_sparse_embeddings=self.use_sparse_embeddings)
            for point in points
        ]
        if scale_score:
    ...
    def _query_hybrid(
        ...
        results = [convert_qdrant_point_to_fiboaitech_document(point, use_sparse_embeddings=True) for point in points]
    ...
    def _set_up_collection(
        ...
        if self.use_sparse_embeddings and not has_named_vectors:
            msg = (
                ...
                f"`fiboaitech_integrations.document_stores.qdrant`."
            )
            raise QdrantStoreError(msg)
    ...
    def _handle_duplicate_documents(
        ...
        Args:
            documents: A list of FiboAITech Document objects.
            index: Name of the index.
            policy: The duplicate policy to use when writing documents.

        Returns:
            A list of FiboAITech Document objects.
        """
