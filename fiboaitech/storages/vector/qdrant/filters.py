...
from fiboaitech.storages.vector.exceptions import VectorStoreFilterException as FilterError
...
def convert_filters_to_qdrant(
    ...
    """Converts FiboAITech filters to the format used by Qdrant.

    Args:
        filter_term: The FiboAITech filter to be converted to Qdrant.
...
def _parse_comparison_operation(
    comparison_operation: str, key: str, value: dict | list | str | float
...
def _build_lt_condition(key: str, value: str | float | int) -> models.Condition:
    if isinstance(value, str) and is_iso_datetime_string(value):
