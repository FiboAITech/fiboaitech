...
from fiboaitech.callbacks.tracing import RunStatus
from fiboaitech.nodes.node import NodeDependency
from fiboaitech.nodes.utils import Output
from fiboaitech.runnables import RunnableConfig, RunnableResult, RunnableStatus
from fiboaitech.utils import format_value
from fiboaitech.utils.utils import JsonWorkflowEncoder, encode_bytes
...
def test_workflow_with_failed_flow(
    ...
    error = ValueError("Error")
    mocker.patch("fiboaitech.flows.flow.Flow._get_nodes_ready_to_run", side_effect=error)
    response = wf.run(
        input_data=input_data,
        config=RunnableConfig(callbacks=[tracing]),
    )
