...
from fiboaitech.nodes.llms import Perplexity
...
def get_perplexity_workflow(
    model: str,
    connection: connections.Perplexity,
):
    wf = Workflow(
        id=str(uuid.uuid4()),
        flow=Flow(
...
def test_workflow_with_perplexity_llm(mock_llm_response_text, mock_llm_executor, model, expected_model):
    model = model
    connection = connections.Perplexity(
        id=str(uuid.uuid4()),
        api_key="api_key",
    )
    wf_perplexity_ai = get_perplexity_workflow(model=model, connection=connection)

    response = wf_perplexity_ai.run(
        input_data={},
    ...
    mock_llm_executor.assert_called_once_with(
        ...
        tool_choice=None,
        model=expected_model,
        messages=wf_perplexity_ai.flow.nodes[0].prompt.format_messages(),
        stream=False,
        temperature=0.1,
        max_tokens=1000,
        stop=None,
