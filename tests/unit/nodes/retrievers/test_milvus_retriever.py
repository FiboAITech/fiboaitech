...
from fiboaitech.components.retrievers.milvus import MilvusDocumentRetriever as MilvusDocumentRetrieverComponent
from fiboaitech.connections import Milvus
from fiboaitech.connections.managers import ConnectionManager
from fiboaitech.nodes.retrievers.base import RetrieverInputSchema
...
def test_initialization_with_defaults():
    ...
    with patch("fiboaitech.connections.Milvus.connect", return_value=mock_milvus_client):
...
def test_execute_with_missing_embedding_key(milvus_document_retriever):
    ...
    with pytest.raises(ValidationError):
        milvus_document_retriever.execute(RetrieverInputSchema(), config)
